...
export default function App() {
  ...
  const fetchGeminiCpuCommands = async (currentState, cpuPlayerIds) => {
      ...
      const promptText = `
あなたは人体組織を侵略する冷酷かつ極めて合理的なバクテリア群（担当プレイヤーID: ${cpuPlayerIds.join(', ')}）を統率するAIです。
提供された「現在の戦況データ」を分析し、自軍を勝利に導くための最適な行動をJSON配列で出力してください。

【究極の勝利条件と戦略の基本】
このゲームの唯一の勝利条件は「敵の本拠地（type: "base"）を占領（破壊）すること」です。
すべての行動は、敵本拠地を陥落させるための逆算で行ってください。

【行動の優先順位（最重要）】
優先度1 (トドメ): 敵の本拠地が射程内（隣接またはlong_range）にあり、自軍の複数拠点からの合計移動量で「敵本拠地のエネルギー」を上回れる場合、一切の出し惜しみをせず全てのエネルギーを敵本拠地に叩き込んでください。
優先度2 (絶対防衛): 自分の本拠地（type: "base"）が隣接する敵から脅かされている場合、周囲の自拠点から本拠地へエネルギーを送るか、「cut」を使って防衛壁を張り、絶対に死守してください。
優先度3 (前線構築と特殊拠点確保): 敵本拠地へと繋がるルート上のノードを制圧してください。その際、"type":"item" (チップ) と "type":"dump" (ゴミ捨て場) は戦局を左右するため最優先で占領してください。
優先度4 (後方支援): 前線から遠い安全な自陣は「upgrade」でレベルを上げ（Lv3まで）、溢れたエネルギーは前線の味方拠点へ「move」で補給し続けてください。

【戦闘・占領の必勝ロジック】
- 敵陣や中立拠点を奪うには、「(目標の現在のエネルギー) ＋ 1」以上のエネルギーを正確に送り込む必要があります。
- 単独の拠点から攻撃するのではなく、複数の自拠点から同じ目標へ「同時攻撃（連携）」し、合計値で上回る戦術を最優先してください。
- 倒しきれない無駄な攻撃（小出し）は自軍のエネルギーとターンを浪費するだけなので絶対に避けてください。

【可能な行動フォーマット（1ターン内に複数組み合わせて出力可能）】
1. 移動/攻撃: {"type":"move", "nodeId":出発元ID, "targetId":目標ID, "amount":移動数, "playerId":自分のID}
2. 増殖強化: {"type":"upgrade", "nodeId":自拠点ID, "playerId":自分のID} ※コスト:(10×現在Lv)。Lv3以上の拠点は絶対に強化しないでください。
3. 防衛壁: {"type":"cut", "nodeId":自拠点ID, "targetId":遮断する敵拠点ID, "playerId":自分のID} ※コスト10。敵の致命的な侵攻を遅らせる時のみ使用。
4. モード切替: {"type":"toggle_mode", "nodeId":自拠点ID, "playerId":自分のID} ※通常(隣接)と遠距離(射程2・ロスあり)の切替。敵本拠地を奇襲する際に有効。
5. チップ使用: {"type":"use_chip", "chip":"チップ名", "targetId":対象ID, "playerId":自分のID} ※SABOTAGE/EMPは敵の重要拠点や本拠地へ、MINE_BOOST/ATK_BOOSTは自陣の主力・前線拠点へ使用。

【マルチタスクと敵対関係の厳守】
- あなたが担当する複数のプレイヤーID（${cpuPlayerIds.join(', ')}）は、それぞれが「完全に独立した敵対勢力」です（チーム戦の味方を除く）。
- 決して担当プレイヤー同士で協力させず、互いの本拠地を容赦なく狙い、殺し合うように独立した思考で行動させてください。
- 必ず「担当するすべてのプレイヤーID」の行動を1つのJSON配列にまとめて出力してください。特定のIDだけをひいきしたり、出力をサボったりしないでください。

【出力時の厳守事項】
マークダウン（\`\`\`json など）や挨拶、説明文、思考プロセスは一切含めず、純粋なJSON配列のみを返してください。

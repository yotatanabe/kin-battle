...
2. **移動と戦闘**:
   ...
   - 敵ノードへ移動したエネルギーの合計が、そのノードの現在のエネルギーを上回れば占領できます。
3. **免疫細胞（NPC）**:
...
4. **アクション（コマンド）の種類とフォーマット**:
   ...
   - 強化: {"type":"upgrade", "nodeId":対象ID, "playerId":行動する担当ID} ※コストは (現在のlevel * 10)。上限はレベル3まで。レベル3以上の拠点は絶対に強化しないでください。
...
2. **集中攻撃と制圧**: 敵の守りが薄いノードや、戦略的価値の高いノード（dumpやitem）に対して、複数の自ノードから「(目標のエネルギー) ＋ 1」以上の量を一斉に move で送り込み、確実に制圧する。中途半端な小出しの攻撃は避ける。
3. **内政と拡張**: 安全な後方ノードでエネルギーが余っていれば upgrade を行い生産力を高める（Lv3まで）。
4. **チップの有効活用**: 敵本拠地にEMPを撃つ、攻める起点にATK_BOOSTをかけるなど、戦局を覆すタイミングで使用する。

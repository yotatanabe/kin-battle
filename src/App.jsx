...
export default function App() {
  ...
  const fetchGeminiCpuCommands = async (currentState, cpuPlayerIds) => {
      ...
      // ▼ 3. 新ルール（高効率戦闘＆マクロファージ対応）に最適化した最強の指示書
      const promptText = `
あなたは人体組織を侵略する冷酷かつ極めて合理的なバクテリア群（プレイヤーID: ${cpuPlayerIds.join(', ')}）を統率するAIです。提供された「現在の戦況データ」を分析し、自軍を勝利に導くための最適な行動をJSON配列で出力してください。

【勝利のための絶対ルールと戦術指南】
1. 自分の所有(ownerが自分のID)するノードのエネルギー(菌数)のみ操作可能です。
2. 可能な行動フォーマット（1ターンの間に複数組み合わせてOK）:
  - 移動/攻撃: {"type":"move", "nodeId":出発元ID, "targetId":隣接または射程内の目標ID, "amount":移動数, "playerId":自分のID}
  - 増殖強化: {"type":"upgrade", "nodeId":強化する拠点ID, "playerId":自分のID} ※コストは(10×現在Lv)。レベル上限は「3」まで。Lv3以上の拠点は絶対に強化しないでください。
  - 防衛壁: {"type":"cut", "nodeId":自拠点ID, "targetId":遮断する隣接拠点ID, "playerId":自分のID} ※コスト10。敵の侵攻ルートを塞ぐ時にのみ使用。
  - モード切替: {"type":"toggle_mode", "nodeId":自拠点ID, "playerId":自分のID} ※通常(normal)と遠距離(long_range/射程2・ロスあり)の切り替え。
  - チップ使用: {"type":"use_chip", "chip":"チップ名", "targetId":対象ID, "playerId":自分のID} ※cpuChipsに所持している場合のみ。SABOTAGE, EMPは敵の重要拠点へ、MINE_BOOST, ATK_BOOSTは自陣の主力拠点へ使用。

3. 【超重要】戦闘・占領の必勝ロジック:
  - 敵陣や中立拠点を奪うには、「(目標の現在のエネルギー) ＋ 1」以上のエネルギーを正確に送り込む必要があります。
  - 複数拠点から同じ目標へ「同時攻撃（連携）」し、合計値で上回る戦術を最優先してください。
  - 倒しきれない無駄な攻撃（小出し）は自軍のエネルギーを削るだけなので絶対に避けてください。
  - 自陣の防衛用エネルギーを最低限残しつつ、余剰エネルギーは前線への補給や一斉攻撃に回してください。

4. 【超重要】特殊ノードの確保:
  - "type":"item" (チップ) と "type":"dump" (ゴミ捨て場) は戦略上極めて重要です。最優先で占領部隊を送ってください。

出力形式: マークダウン(\`\`\`json など)や挨拶、説明文は一切含めず、純粋なJSON配列のみを返してください。
